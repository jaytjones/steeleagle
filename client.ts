...
// --------------------------------------------------------
// Trader WRITE path (v2.0 order placement) — deliberately separate from
// schwabFetch. Order writes legitimately return EMPTY bodies on success
// (POST /orders → 201 + empty body + the order id in the Location header;
// DELETE /orders/{id} → empty 200), so the empty-body guard above — which
// exists to catch the stale-hash signature on READS — must not apply here.
// Success/failure is judged on the HTTP status alone.
// --------------------------------------------------------
export interface TraderWriteResult {
  status: number
  /** The Location response header — POST /orders returns the new order URL here. */
  location: string | null
  /** Raw body text (usually empty on success; error JSON on failure). */
  body: string
}

export async function traderWrite(
  method: 'POST' | 'PUT' | 'DELETE',
  path: string,
  body?: unknown,
): Promise<TraderWriteResult> {
  const accessToken = await getAccessToken()

  const response = await fetch(`${TRADER_BASE}${path}`, {
    method,
    headers: {
      Authorization: `Bearer ${accessToken}`,
      Accept: 'application/json',
  ...
  if (!response.ok) {
    throw new Error(
      `Schwab API error ${response.status} on ${method} ${path}: ${text || '(empty body)'}`,
    )
  }
