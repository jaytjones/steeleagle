# Iron Condor Strategy - Version 1.5
...
## 9. Addendum — Scaling the Core: Larger Bankroll & Higher-Delta Short Legs (Exploratory)

> **Status of this section.** This addendum is **exploratory and model-derived**, not a change to the operative rules. Sections 1–8 remain the live strategy at the current $10k bankroll. Everything below describes what a Monte Carlo simulation of the core strategy suggests *if and when* the account scales toward ~$30k and *if* you choose to push the short-leg delta past the 16Δ standard. The numbers are simulation outputs under explicit assumptions (Section 9.7), not forecasts. **Paper-trade or backtest any change here before committing real capital.**

### 9.1 The Question
The core (Sections 2–7) places short legs at ~16Δ — a 1-standard-deviation strike that targets a ~68% probability of expiring between the shorts. Two natural questions arise as the account grows:

1. **Can the short-leg delta be raised** (strikes closer to the money) and still keep a positive-expectancy trade — and do returns improve if so?
2. **Does a larger bankroll make higher-delta trading more worthwhile**, or does it just scale the same outcome?

The short answer: raising delta keeps the trade positive-EV well past 16Δ and *raises* expected return up to a peak, but the ceiling is set by the **variance risk premium (VRP)** — the gap between implied and realized volatility — not by the delta itself. And a bigger bankroll amplifies the higher-delta payoff, but **only** through a deployment (concurrency) decision that simultaneously concentrates tail risk.

### 9.2 Higher-Delta Short Legs — Per-Trade Economics
As the short delta rises, the strikes sit closer to the money: credit collected rises, the break-evens widen (the larger credit cushions more near-misses into wins), and because **max loss = wing − credit**, the BPR per contract *falls*. Win rate erodes only gently. Expected value per trade rises to a peak and then declines as the win-rate erosion finally outruns the credit growth.

Simulated per-trade economics, $10-wide wings, 40→21 DTE, 50% profit / 2× stop, calibrated so 16Δ matches the established $10k baseline:

| Short Δ | Credit | Credit / width | BPR / contract | Win rate | EV / trade |
| :--- | ---: | ---: | ---: | ---: | ---: |
| **16Δ** *(standard)* | $222 | 22% | $778 | 68.0% | $11 |
| **20Δ** | $285 | 29% | $715 | 67.0% | $15 |
| **25Δ** | $366 | 37% | $634 | 66.3% | **$17–18 (peak)** |
| **30Δ** | $450 | 45% | $550 | 65.3% | $16–17 |
| **35Δ** | $537 | 54% | $463 | 64.9% | $15–16 |
| **40Δ** | $626 | 63% | $374 | 63.5% | $12 |

> **Key finding — the ceiling is the VRP, not the delta.** EV/trade does not cross into negative territory as strikes move toward the money. Across *every* delta, expected value goes to zero only when realized volatility reaches roughly **90–94% of implied** (i.e., when IV stops meaningfully overstating the move). Above that ratio the trade is negative-EV regardless of strike placement; below it, it is positive almost everywhere. **The IV Rank > 25% entry filter (Section 3) is what keeps you in the rich-VRP regime where any of this works.** If the VRP is thin, stay at 16Δ — or don't trade.

### 9.3 Wider Wings at Scale — The "Free" Improvement
At ~$30k, the BPR-scaling rule in Section 4 (≈5% of capital ≈ $1,500/position) naturally moves the structure to **$20-wide wings** — one $20-wide condor occupies one position slot. Because commission is charged **per contract, not per dollar of width** (Section 7), widening the wings spreads the fixed $5.20 round-trip across a larger credit and **roughly halves commission friction** with no increase in directional risk:

| Short Δ | $10-wing friction (% of win) | $20-wing friction (% of win) | EV multiple ($20w ÷ $10w) |
| :--- | ---: | ---: | ---: |
| 16Δ | 4.7% | 3.3% | 1.85× |
| 25Δ | 2.8% | 1.9% | 1.81× |
| 30Δ | 2.3% | 1.6% | **2.35×** |
...
This friction dilution is the genuinely free part of scaling — you capture it at any delta simply by using wider wings, and it rewards higher-delta condors disproportionately (their large credit pairs with proportionally tiny friction). Note that the **credit-to-width ratio falls** with wider wings (the further-out long leg is cheap protection), so the 15% credit/width floor in Section 5 should be read as a $10-wing convention; on $20 wings a lower ratio is expected and acceptable.

### 9.4 Deployment Regimes — Why the Bankroll Alone Doesn't Help
Simulated annual outcomes ($20 wings at $30k vs the $10k/$10-wing base; medians, with the 10th-percentile "typical bad year" and worst simulated year):

| Short Δ | $10k (5-pos, 80 tr/yr) median | $30k **conservative** (5-pos, 80 tr/yr) median | $30k **scaled** (~8 concurrent, 160 tr/yr) median | $30k scaled — worst year |
| :--- | ---: | ---: | ---: | ---: |
| 16Δ | 6.2% | 4.2% | 9.0% | −27% |
| 20Δ | 9.7% | 5.6% | 11.7% | −31% |
| 25Δ | 10.8% | 6.4% | 13.4% | −40% |
| **30Δ** | 10.1% | 8.6% | **18.1% (peak)** | −35% |
| 35Δ | 10.2% | 8.0% | 16.6% | −34% |
| 40Δ | 7.9% | 7.4% | 15.3% | −36% |

Three findings:

1. **The return-optimal delta shifts higher with scale** — from ~25Δ on the $10k account to **~30Δ** in the scaled $30k regime. The mechanism is mechanical: higher-delta condors carry lower BPR per contract, so when there is buying-power headroom to fill, the bigger account can hold more of them per dollar. Scale rewards the exact trait — low per-contract BPR — that higher delta provides.

2. **A bigger bankroll, run conservatively, earns a *lower* percentage return** than the small account (the middle column). A 5-position cap leaves $30k deploying only ~25% of capital; you have tripled the denominator without tripling the work. The headline percentage gains appear **only** in the scaled column, which uses the buying-power headroom by running ~8 concurrent positions and trading ~3×/week. **The "$30k is better" story is a deploy-more-concurrent-capital story** — it is a leverage decision, not a free benefit of account size.

3. **The friction dilution (Section 9.3) is the only part that is unambiguously free.** The return scaling above it is paid for in risk, below.

### 9.5 The Risk Side — Where the Returns Actually Come From
The scaled regime's *typical* year is smooth (its 10th-percentile outcome is near break-even or slightly positive, because the fat credit cushions ordinary bad runs), but its **worst simulated year runs −27% to −40%.** This is the quantified version of the correlation warnings in Section 3: eight concurrent condors usually diversify well, but when a systemic shock catches all of them open at once, defined-risk stops being comforting and the account takes an equity-like drawdown. The distribution goes **bimodal** — better most years, occasionally severe.

> ⚠️ **Diversification supply — not buying power — is the true binding constraint.** The ~8-concurrent scaled curve assumes eight *genuinely uncorrelated* setups exist. But the universe is five pillars across 21 instruments with hard per-pillar limits (Equity block max 2; Volatility max 1; Currency max 1). Pushing concurrency past what the pillars supply forces you to **double up within a pillar**, which breaks the uncorrelated-shock assumption the model relies on — meaning the real −35% tail at high concurrency is likely **worse and more frequent** than the table shows. You cannot safely fill the scaled curve unless you genuinely have the uncorrelated pillars to fill it.

> ⚠️ **Stops provide less protection at high delta.** On a high-credit condor, the 2× stop (Section 5) sits very close to max loss, so it rarely triggers before a large adverse move has already happened. The closer the short sits to the money, the more the position is governed by gap risk and the less the stop actually defends. Treat higher-delta short legs as having *weaker* mechanical risk control, and consider a tighter stop multiple (toward the 1.5× used for the Volatility pillar) if pushing past ~25Δ.

### 9.6 Recommended Posture at Scale
If and when the account scales toward $30k:

* **Take the free gain.** Move to wider wings (≈$20) per the Section 4 BPR-scaling rule. This dilutes commission friction at every delta with no added directional risk.
* **Bump the delta modestly, not aggressively.** The defensible improvement over 16Δ is the **20–25Δ band** — it captures meaningful additional EV and return while staying firmly in high-probability territory. The 30Δ/18% peak is achievable on paper but is a concentrated bet wearing an income strategy's clothing.
* **Let diversification supply cap concurrency, not buying power.** Increase concurrent positions only as far as you have genuinely uncorrelated pillars to fill, respecting the per-pillar limits in Section 3. In practice this caps you well short of the idealized 8-position scaled curve.
* **Keep every management rule in Sections 4–7 intact** — 50% profit target, 21-DTE exit, BPR cap, laddering — and tighten stops if trading above ~25Δ (Section 9.5).
* **Re-verify the VRP every entry.** The entire higher-delta case is contingent on IV continuing to overstate realized vol. The IV Rank > 25% filter is the gate; do not push delta in low-VRP conditions.

### 9.7 Model Assumptions & Caveats
These results are simulation outputs, not guarantees. They depend on:

* **Variance risk premium** calibrated so realized vol ≈ 84% of implied (IV overstating by ~18%), biased rich by the IVR > 25% entry filter. Thinner VRP shrinks the positive-EV region toward lower deltas.
* **Symmetric volatility surface.** The model treats put and call sides roughly symmetrically. Real equity-index skew makes the **put side at higher delta meaningfully more dangerous** than shown — a material caveat for Equity-pillar names.
* **Gap risk** modeled as occasional overnight jumps; **correlated drawdowns** modeled as rare systemic shocks hitting concurrent positions. Both are approximations; real regime-clustering (multiple stop-outs in one month) may exceed the model.
* **Independent-setup assumption** in the scaled regime, which — as Section 9.5 stresses — does not hold once you double up within a pillar.
* **This is a low-Sharpe strategy at every delta** (per-trade reward-to-variability never exceeds ~0.13). Changing the delta reshuffles the return/risk trade-off; it does not transform the strategy into a high-conviction edge.

> ⚠️ **Not financial advice.** This addendum is a planning exercise for a single operator's own capital. Validate against your own broker's historical data, paper-trade the higher-delta and wider-wing structures, and confirm the per-pillar correlation behavior on live fills before scaling real money into any configuration described here.

---
**Version:** 1.5
**Last Updated:** May 20, 2026
**Changes from 1.4:** Added **Section 9 — Addendum: Scaling the Core (Exploratory)**, a model-derived exploration of running the core strategy at a larger (~$30k) bankroll with higher-delta short legs. Covers per-trade economics of raising the short delta past the 16Δ standard (EV peaks ~25Δ; positive-EV ceiling set by the variance risk premium, not the delta), the commission-friction dilution from wider ($20) wings at scale, the deployment-regime finding that a bigger bankroll only raises percentage returns if concurrent capital is increased (shifting the return-optimal delta to ~30Δ), and the corresponding tail-risk concentration (worst simulated year −27% to −40%), with diversification supply — not buying power — identified as the true binding constraint. Section is explicitly exploratory and does not alter the operative $10k rules in Sections 1–8. No changes to Sections 1–8.
